...
ParamUnitName = OpenApiParameter(
    ...
    required=True,
    location="path",
    description="نام یونیت systemd (مثال: nginx یا nginx.service)",
)
...
class SystemServiceViewSet(viewsets.ViewSet, ServiceValidationMixin):
    ...
    @extend_schema(
        parameters=[ParamStateFilter],
        responses={200: inline_serializer("UnitList", {"data": serializers.JSONField()})}
    )
    ...
    @extend_schema(
        parameters=[ParamUnitName],
        responses={200: inline_serializer("UnitDetail", {"data": serializers.JSONField()})}
    )
    ...
    @extend_schema(
        parameters=[ParamUnitName, ParamAction],
    ...
    def control_service(self, request: Request, unit_name: str) -> Response:
        ...
        request_data = {"action": [action_name]} if action_name else {}
        if not action_name:
        ...
        try:
            ...
            method(unit_name)  # نام خام ارسال می‌شود؛ نرمالایز درون ServiceManager انجام می‌شود
            return StandardResponse(
