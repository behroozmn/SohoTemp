...
@method_decorator(csrf_exempt, name='dispatch')
class PoolDatasetsView(APIView):
...
@method_decorator(csrf_exempt, name='dispatch')
class PoolDeviceNamesView(APIView):
...
@method_decorator(csrf_exempt, name='dispatch')
class PoolFeaturesView(APIView):
...
class PoolCreateView(APIView):
    ...
    NOT IMPLEMENTED in current PoolManager, but ready for extension.
    Expected body:
    {
        "name": "newpool",
