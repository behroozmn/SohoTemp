...
و در جاهایی که «اِی پی آی» کامل در دسترس نیست با fallback ایمن به «کامندلاین»های رسمی («زِدپول یا زِد اِف اِس») عملیات روی «زِد اِف اِس» را انجام می‌دهد.
...
- رویکرد: ابتدا سعی در استفاده از libzfs برای کارهای introspection و set/get property، و هرجا لازم باشد
  اجرای امن «کامندلاین» بدون shell=True برای جلوگیری از تزریق فرمان.
- مخاطب: توسعه‌دهندگان مبتدی تا پیشرفته؛ برای مبتدی‌ها داک‌استرینگ‌ها و کامنت‌های فارسی مفصل ارائه شده است.
- ملاحظات امنیتی: عمیات مخرب مانند destroy و rollback خطرناک‌اند؛ در محیط تولید باید احراز هویت/مجوزدهی مناسب در «دی آر اِف» اعمال شود.
...
import subprocess  # اجرای امن فرمان‌های سیستمی («زِد اِف اِس»/«زِد پول») وقتی «اِی پی آی» کامل نیست یا برای گزارش خام.
...
# --------------------------- JSON envelopes ---------------------------  # الگوی یکسان پاسخ برای «دی آر اِف» (موفق/ناموفق).

def ok(data: Any, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # تابع کمکی برای ساخت پاسخ موفق استاندارد.
    """
    این تابع یک پوشش (envelope) استاندارد برای پاسخ موفق تولید می‌کند تا به‌صورت مستقیم در «دی آر اِف» استفاده شود.
    ورودی‌ها:
    ...
    نکته:
      - این الگو با Response() در «دی آر اِف» سازگار است و سریالایز می‌شود.
...
def fail(message: str, code: str = "zfs_error", extra: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # تابع کمکی برای ساخت پاسخ خطا.
    """
    این تابع ساختار یک پاسخ خطا را ایجاد می‌کند تا خطاها به‌صورت یکنواخت به کلاینت بازگردند.
    ورودی‌ها:
    ...
    خطاها:
      - ندارد؛
       فقط یک ساختار ثابت تولید می‌شود.
...
class ZFSError(Exception):  # استثنای اختصاصی دامنهٔ «زِد اِف اِس» برای تمایز خطاها در لاجیک برنامه.
    """
    استثنای دامنه‌ای برای مدیریت یک‌دست خطاهای مرتبط با عملیات «زِد اِف اِس».
    چرا؟ برای اینکه بتوانیم خطاهای مربوط به «زِد اِف اِس» را از سایر خطاها جدا کنیم و در handlerهای «دی آر اِف» واکنش مناسب نشان دهیم.
    ورودی‌ها/خروجی:
...
class ZFSManager:  # کلاس مدیریت سطح‌بالای «زِد اِف اِس» با خروجی JSON-Ready و fallback به «کامندلاین».
    """
    هدف: ارائهٔ «اِی پی آی» سطح‌بالا برای مدیریت «زِد اِف اِس» که
      1) تا جای ممکن از libzfs برای کارهای داخل حافظه (introspection و set/get properties) استفاده کند.
      2) در جاهایی که «اِی پی آی» پوشش ندارد (مثل send/receive یا create pool)، با اجرای امن «کامندلاین» («زِدپول یا زِد اِف اِس») کار را انجام دهد.
      3) تمام خروجی‌ها را به‌صورت JSON-Ready برگرداند تا با «دی آر اِف» به‌راحتی قابل مصرف باشد.
    ...
    نکات مهم:
      ...
      - پارامتر run_timeout: محدودیت زمانی اجرای فرمان‌های «کامندلاین» (ثانیه).
      - برای عملیات خطرناک (destroy, rollback) حتماً احراز هویت و مجوزدهی سمت «اِی پی آی» را جدی بگیرید.
    خروجی متدها:
    ...
    def __init__(self, dry_run: bool = False, run_timeout: int = 180) -> None:  # سازندهٔ کلاس با تنظیمات اجرایی.
        """
        سازندهٔ کلاس که اتصال به libzfs را برقرار می‌کند و تنظیمات پایه را می‌گیرد.
        ورودی‌ها:
          - dry_run (bool): اگر True باشد، عملیات تغییر حالت سیستم (مثل create/destroy) اجرا نمی‌شود و فقط لاگ/خروجی ساختگی تولید می‌شود.
          - run_timeout (int): محدودیت زمانی (ثانیه) برای اجرای فرمان‌های «کامندلاین»؛ از هنگ کردن فرآیند جلوگیری می‌کند.
        خروجی:
        ...
        """
        self.zfs = libzfs.ZFS()  # ایجاد نمونهٔ اصلی libzfs برای دسترسی به پول و دیتاستها.
        self.dry_run = dry_run  # ذخیرهٔ وضعیت dry-run برای استفاده در متدهای تغییر حالت.
        self.run_timeout = run_timeout  # ذخیرهٔ محدودیت زمانی اجرای «کامندلاین» برای همهٔ فراخوانی‌های _run.

    # --------------------------- internal helpers ---------------------------  # توابع کمکی داخلی؛ خارج از «اِی پی آی» عمومی.

    def _run(self, args: List[str], stdin: Optional[bytes] = None) -> Tuple[str, str]:  # اجرای امن فرمان «کامندلاین» با آرگومان‌های لیستی.
        """
        این متد داخلی فرمان‌های «کامندلاین» مثل «زِدپول یا زِد اِف اِس» را به‌صورت ایمن اجرا می‌کند.
    ...
    def _get_dataset(self, name: str) -> libzfs.ZFSDataset:  # دریافت شیء دیتاست از libzfs.
        """
        تلاش برای دریافت یک دیتاست با نام مشخص از libzfs.
        ورودی:
          - name (str): نام کامل دیتاست (مثل "tank/data" یا "tank/vol1").
        خروجی:
        ...
        """
        try:
            return self.zfs.get_dataset(name)  # فراخوانی مستقیم «اِی پی آی» libzfs برای دریافت دیتاست.
    ...
    def _get_pool(self, name: str) -> libzfs.ZFSPool:  # دریافت شیء «زِد پول» بر اساس نام.
        """
        در بین «زِد پول»ها پیمایش می‌کند و پول با نام داده‌شده را برمی‌گرداند.
        ورودی:
          - name (str): نام «زِد پول» (مثل "tank").
        خروجی:
          - libzfs.ZFSPool: شیء پول اگر یافت شود.
        خطاها:
          - ZFSError: اگر پول با نام مشخص وجود نداشته باشد.
        ...
        for pool in self.zfs.pools:  # پیمایش لیست poolها از libzfs.
            if pool.name == name:  # بررسی تطابق نام.
                return pool  # بازگرداندن پول مورد نظر.
    ...
    def _safe_prop_value(self, v: Any) -> Any:  # نرمال‌سازی مقدار پراپرتی libzfs به مقدار ساده.
        """
        برخی bindingها به‌جای مقدار خام، یک شیء Property برمی‌گردانند؛ این تابع در صورت وجود صفت value آن را استخراج می‌کند.
        ورودی:
    ...
    def list_pools(self) -> Dict[str, Any]:  # لیست نام تمام «زِد پول»ها.
        """
        نام تمام «زِد پول»های موجود در سیستم را از libzfs می‌خواند.
        ورودی:
          - ندارد.
        خروجی:
    ...
    def pool_status(self, pool: str) -> Dict[str, Any]:  # وضعیت سادهٔ یک «زِد پول».
        """
        وضعیت پایهٔ یک «زِد پول» شامل نام، state، health، guid و تعدادی پراپرتی رایج را برمی‌گرداند.
        ورودی:
          - pool (str): نام «زِد پول» (مثلاً "tank").
        خروجی:
          - dict(JSON-Ready): شامل فیلدهای کلیدی و dict پراپرتی‌ها.
        خطاها:
          - ZFSError: اگر پول پیدا نشود؛ در خروجی fail(...) بازتاب می‌یابد.

        EN: Basic zpool status and a few properties.
        """
        try:
            p = self._get_pool(pool)  # بازیابی شیء پول.
            extras = {}  # دیکشنری برای پراپرتی‌های اضافه.
            for prop in ("ashift", "autoexpand", "autoreplace", "autotrim", "listsnapshots"):  # پراپرتی‌های متداول.
                try:
                    if hasattr(p, "get_prop"):  # اگر «اِی پی آی» گرفتن پراپرتی وجود دارد.
    ...
    def pool_status_verbose(self, pool: str) -> Dict[str, Any]:  # خروجی خام «زِد پول» status -v برای عیب‌یابی.
        """
        خروجی کامل و خام `zpool status -v` را بازمی‌گرداند که برای عیب‌یابی دقیق (device errors, checksum, ...) کاربرد دارد.
        ورودی:
          - pool (str): نام «زِد پول».
        خروجی:
          - dict(JSON-Ready): {"ok": True, "data": {"raw": "<full text>"}}
        خطاها:
          - ZFSError: در صورت خطا در اجرای «کامندلاین».
    ...
    def pool_iostat(self, pool: Optional[str] = None, samples: int = 1, interval: int = 1) -> Dict[str, Any]:  # نمونهٔ I/O stats.
        """
        یک عکس لحظه‌ای از آمار I/O با `zpool iostat -v` می‌گیرد. برای پایش سریع کاراست.
        ورودی‌ها:
        ...
        try:
            ...
            if pool:  # اگر نام پول داده شده باشد، اضافه کن.
                args.append(pool)
            args += [str(samples), str(interval)]  # افزودن پارامترهای نمونه و بازه.
            out, _ = self._run(args)  # اجرای امن «کامندلاین».
            return ok({"raw": out})
        except Exception as exc:
            return fail(str(exc))
    ...
    def list_datasets(self, pool: Optional[str] = None,
        ...
        """
        فهرست دیتاست‌ها (فایل‌سیستم, volume(zvol), اسنپ‌شات) را به همراه نوعشان بازمی‌گرداند.
        ورودی‌ها:
          - pool (str|None): اگر مشخص شود، فقط دیتاست‌های زیر آن پول لیست می‌شوند.
        ...
        خطاها:
          - ZFSError: در صورت خطا در اجرای «کامندلاین» یا پارس خروجی.
        ...
        try:
            ...
            if pool:
                args.append(pool)  # محدودکردن به یک پول خاص.
            ...
            for line in out.splitlines():  # پیمایش هر خط از خروجی.
                ...
                if pool is None or name_i.split("/")[0] == pool:  # در صورت نبود فیلتر پول، همه را می‌پذیریم.
    ...
    def get_props(self, target: str) -> Dict[str, Any]:  # گرفتن همهٔ پراپرتی‌ها برای یک دیتاست.
        """
        تمام پراپرتی‌های قابل مشاهدهٔ یک دیتاست را (ترجیحاً با libzfs و در غیر اینصورت با «کامندلاین») بازمی‌گرداند.
        ورودی:
          - target (str): نام دیتاست (مانند "tank/data" یا "tank/vol").
        خروجی:
          - dict(JSON-Ready): map پراپرتی به مقدار آن (همه رشته).
        خطاها:
          - ZFSError: در صورت خطا در libzfs یا اجرای «کامندلاین».
        ...
        try:
            try:
                ...
                pass  # اگر libzfs شکست خورد، به «کامندلاین» می‌رویم.
            out, _ = self._run(["zfs", "get", "-H", "-o", "property,value", "all", target])  # خواندن همهٔ پراپرتی‌ها با «کامندلاین».
            props: Dict[str, Any] = {}
    ...
    # --------------------------- pool operations ---------------------------  # متدهای ساخت/حذف/وضعیت پول.

    def create_pool(self, name: str, vdevs: List[List[str]],
                    properties: Optional[Dict[str, str]] = None,
                    force: bool = False, altroot: Optional[str] = None,
                    ashift: Optional[int] = None) -> Dict[str, Any]:  # ساخت «زِد پول» جدید با کامندلاین.
        """
        یک «زِد پول» جدید می‌سازد. برای vdevها باید آرایه‌ای از گروه‌ها بدهید (مثلاً [["mirror","/dev/sdb","/dev/sdc"], ["raidz1",...]]).
        ورودی‌ها:
          - name (str): نام «زِد پول» (مثلاً "tank").
          - vdevs (List[List[str]]): گروه‌های vdev به همان ترتیبی که در «کامندلاین» می‌دهید (مانند mirror/raidz/diskها).
          - properties (dict|None): پراپرتی‌های سطح پول (مثل {"autoexpand": "on"}).
        ...
        خطاها:
          - ZFSError: اگر اجرای «کامندلاین» شکست بخورد (مثلاً دیسک‌ها در استفاده باشند).

        EN: Create a zpool using CLI. Provide vdev groups just like zpool create syntax.
        """
        try:
            args = ["zpool", "create"]  # آغاز آرگومان‌های ساخت پول.
            if force:
    ...
    def destroy_pool(self, name: str, force: bool = False) -> Dict[str, Any]:  # حذف «زِد پول» (خطرناک).
        """
        یک «زِد پول» را به‌طور کامل حذف می‌کند. بسیار خطرناک است و همهٔ داده‌ها از دست می‌رود.
        ورودی‌ها:
          - name (str): نام پول که باید حذف شود.
          - force (bool): استفاده از -f برای اجبار.
        خروجی:
          - dict(JSON-Ready): نتیجهٔ عملیات با stdout.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین».
    ...
    def import_pool(self, name: Optional[str] = None,
                    dir_hint: Optional[str] = None, readonly: bool = False) -> Dict[str, Any]:  # ایمپورت پول.
        """
        ایمپورت یک «زِد پول» از دیسک‌ها. می‌توانید مسیر جستجو بدهید یا حالت فقط‌خواندنی تنظیم کنید.
        ورودی‌ها:
          - name (str|None): نام پول برای ایمپورت؛ اگر None باشد لیست ایمپورت‌ها را نشان می‌دهد/همه را ایمپورت می‌کند.
    ...
    def export_pool(self, name: str, force: bool = False) -> Dict[str, Any]:  # اکسپورت پول.
        """
        «زِد پول» را از سیستم جاری خارج (export) می‌کند تا در سیستم دیگری import شود.
        ورودی‌ها:
          - name (str): نام پول.
    ...
    def scrub_pool(self, name: str, stop: bool = False) -> Dict[str, Any]:  # شروع/توقف scrub.
        """
        اسکراب را برای بررسی و اصلاح silent errorها در پول آغاز یا متوقف می‌کند.
        ورودی‌ها:
          - name (str): نام پول.
    ...
    def clear_pool(self, name: str, device: Optional[str] = None) -> Dict[str, Any]:  # پاک‌سازی شمارنده‌های خطا.
        """
        شمارنده‌های خطا را در سطح پول یا یک وسیلهٔ خاص پاک می‌کند (zpool clear).
        ورودی‌ها:
          - name (str): نام پول.
    ...
    def features(self, pool: str) -> Dict[str, Any]:  # لیست feature@* ها.
        """
        با استفاده از `zpool get all` فهرست feature@* و وضعیتشان را استخراج می‌کند.
        ورودی:
          - pool (str): نام پول.
        خروجی:
    ...
    # --------------------------- dataset operations ---------------------------  # متدهای مدیریت دیتاست.

    def create_dataset(self, name: str, properties: Optional[Dict[str, str]] = None,
                       dataset_type: str = "filesystem") -> Dict[str, Any]:  # ایجاد فایل‌سیستم یا zvol.
        """
        یک دیتاست جدید می‌سازد؛ اگر نوع "والیوم" انتخاب شود، باید حتماً پراپرتی "volsize" تعیین شود.
        ورودی‌ها:
          ...
          - dataset_type (str): "فایل‌سیستم" یا "والیوم". پیش‌فرض "فایل‌سیستم".
        خروجی:
          - dict(JSON-Ready): نتیجهٔ ساخت با stdout.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» (مانند نبودن volsize برای والیوم).
    ...
    def destroy_dataset(self, name: str, recursive: bool = False, force: bool = False) -> Dict[str, Any]:  # حذف دیتاست.
        """
        دیتاست را حذف می‌کند؛ با گزینهٔ recursive می‌توانید فرزندان/اسنپ‌شات‌ها را هم حذف کنید.
        ورودی‌ها:
          - name (str): نام دیتاست.
    ...
    def set_props(self, target: str, properties: Dict[str, str]) -> Dict[str, Any]:  # ست‌کردن پراپرتی‌ها.
        """
        پراپرتی‌های یک دیتاست را تنظیم می‌کند؛ ابتدا سعی با libzfs و در صورت نیاز با «کامندلاین».
        ورودی‌ها:
    ...
    def snapshot(self, name: str, recursive: bool = False, props: Optional[Dict[str, str]] = None) -> Dict[str, Any]:  # ساخت اسنپ‌شات.
        """
        یک اسنپ‌شات به فرم <dataset>@<snap> ایجاد می‌کند؛ می‌توانید recursive و پراپرتی‌های لازم را بدهید.
        ورودی‌ها:
          - name (str): نام اسنپ‌شات مثل "tank/data@before-deploy".
          - recursive (bool): اگر True، روی زیرمجموعه‌ها هم اعمال می‌شود (-r).
          - props (dict|None): پراپرتی‌های اختیاری در زمان اسنپ‌شات (با -o key=value).
        خروجی:
          - dict(JSON-Ready): نتیجهٔ ساخت.
        خطاها:
          - ZFSError: خطای اجرای «کامندلاین».
    ...
    def list_snapshots(self, dataset: Optional[str] = None) -> Dict[str, Any]:  # فهرست اسنپ‌شاتها.
        """
        لیست اسنپ‌شاتها را با ستون‌های مفید (name, creation, used, refer) بازمی‌گرداند.
        ورودی:
          - dataset (str|None): اگر None باشد، همهٔ اسنپ‌شاتهای سیستم لیست می‌شوند.
        خروجی:
    ...
    def bookmark(self, snapshot: str, bookmark: str) -> Dict[str, Any]:  # ساخت بوک‌مارک از اسنپ‌شات.
        """
        یک بوک‌مارک سبک از اسنپ‌شات می‌سازد که برای ریپلیکیشن و مرجع‌گذاری مفید است.
        ورودی‌ها:
          - snapshot (str): نام اسنپ‌شات مبدأ (مانند "tank/data@pre").
          - bookmark (str): نام بوک‌مارک مقصد (مانند "tank/data#pre").
        خروجی:
          - dict(JSON-Ready): نتیجهٔ ساخت.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین».
    ...
    def list_bookmarks(self, dataset: Optional[str] = None) -> Dict[str, Any]:  # لیست بوک‌مارکها.
        """
        تمام بوک‌مارکها را (یا زیر یک دیتاست مشخص) لیست می‌کند.
        ورودی:
          - dataset (str|None): اگر مشخص شود، با -r زیر همان دیتاست جستجو می‌شود.
        خروجی:
          - dict(JSON-Ready): لیست اسامی بوک‌مارکها.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین».
    ...
    def clone(self, snapshot: str, target: str, properties: Optional[Dict[str, str]] = None) -> Dict[str, Any]:  # کلون از اسنپ‌شات.
        """
        از اسنپ‌شات یک دیتاست نوشتنی جدید می‌سازد (clone) که برای تست/ایزوله‌سازی تغییرات مفید است.
        ورودی‌ها:
          - snapshot (str): نام اسنپ‌شات مبدأ (مثل "tank/data@pre").
    ...
    def promote(self, dataset: str) -> Dict[str, Any]:  # promote کلون برای مستقل‌سازی.
        """
        کلون را به دیتاست معمولی تبدیل می‌کند تا وابستگی به والد قطع شود (zfs promote).
        ورودی:
    ...
    def rename(self, src: str, dst: str, recursive: bool = False) -> Dict[str, Any]:  # تغییر نام دیتاست.
        """
        نام دیتاست را تغییر می‌دهد؛ می‌توان به‌صورت بازگشتی نیز عمل کرد.
        ورودی‌ها:
          - src (str): نام فعلی.
          - dst (str): نام جدید.
    ...
    def rollback(self, dataset: str, to_snapshot: Optional[str] = None,
                 destroy_more_recent: bool = False) -> Dict[str, Any]:  # بازگشت به اسنپ‌شات.
        """
        دیتاست را به یک اسنپ‌شات مشخص یا آخرین اسنپ‌شات بازمی‌گرداند. می‌توان اسنپ‌شاتهای جدیدتر را نیز حذف کرد.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
          - to_snapshot (str|None): نام اسنپ‌شات هدف (با یا بدون "dataset@"؛ هر دو پذیرفته می‌شود).
          - destroy_more_recent (bool): اگر True، -r اضافه می‌شود تا اسنپ‌شات های جدیدتر حذف شوند.
        خروجی:
    ...
    def mount(self, dataset: str) -> Dict[str, Any]:  # مونت دیتاست.
        """
        دیتاست نوع فایل‌سیستم را طبق mountpoint خودش سوار می‌کند.
        ورودی:
          - dataset (str): نام دیتاست.
        خروجی:
    ...
    def unmount(self, dataset: str, force: bool = False) -> Dict[str, Any]:  # آن‌مونت دیتاست.
        """
        دیتاست را پیاده می‌کند؛ در صورت نیاز با -f.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def set_quota(self, dataset: str, size: str) -> Dict[str, Any]:  # تعیین quota.
        """
        محدودیت فضا (quota) را روی دیتاست تنظیم می‌کند؛ مثل "100G" یا "none".
        ورودی‌ها:
          - dataset (str): نام دیتاست.
        ...
        خطاها:
          - ZFSError: مقدار نامعتبر یا خطای «کامندلاین».
    ...
    def set_refquota(self, dataset: str, size: str) -> Dict[str, Any]:  # تعیین refquota.
        """
        محدودیت فضا بر مبنای فضای referenced.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
        ...
        خروجی:
          - dict(JSON-Ready): نتیجهٔ تنظیم.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» یا مقدار نامعتبر.
    ...
    def set_reservation(self, dataset: str, size: str) -> Dict[str, Any]:  # تعیین reservation.
        """
        رزرو فضای تضمین‌شده برای دیتاست (reservation) را ست می‌کند.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def set_refreservation(self, dataset: str, size: str) -> Dict[str, Any]:  # تعیین refreservation.
        """
        رزرو بر اساس referenced space را تنظیم می‌کند.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def list_user_quotas(self, dataset: str) -> Dict[str, Any]:  # لیست سهمیه‌های کاربر/گروه.
        """
        با استفاده از `zfs userspace` و `zfs groupspace` سهمیه‌ها و مصرف کاربران/گروه‌ها را گزارش می‌کند.
        ورودی:
          - dataset (str): نام دیتاست هدف.
        خروجی:
          - dict(JSON-Ready): {"users":[{"name":...,"used":...,"quota":...},...], "groups":[...]}.
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» یا پارس خروجی.
    ...
    def enable_compression(self, dataset: str, algo: str = "lz4") -> Dict[str, Any]:  # فعال‌سازی compression.
        """
        فشرده‌سازی را برای دیتاست فعال می‌کند. الگوریتم‌ها: lz4, zstd, gzip, off.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def enable_dedup(self, dataset: str, mode: str = "on") -> Dict[str, Any]:  # فعال‌سازی deduplication.
        """
        deduplication را فعال/غیرفعال می‌کند. مقادیر مجاز: on, verify, off.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def set_record_or_volblock(self, dataset: str, size: str = "128K") -> Dict[str, Any]:  # تعیین recordsize/volblocksize.
        """
        اگر دیتاست از نوع zvol باشد volblocksize تنظیم می‌شود و اگر فایل‌سیستم باشد recordsize.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def set_mountpoint(self, dataset: str, path: str) -> Dict[str, Any]:  # تعیین mountpoint.
        """
        مسیر mountpoint دیتاست را تنظیم می‌کند.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
          - path (str): مسیر مقصد.
        خروجی:
    ...
    def set_atime(self, dataset: str, mode: str = "off") -> Dict[str, Any]:  # تنظیم atime.
        """
        روشن/خاموش کردن atime برای کاهش writeهای اضافی.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
          - mode (str): "on" یا "off".
        خروجی:
    ...
    def send(self, snapshot: str, incremental_from: Optional[str] = None, raw: bool = False,
        ...
        """
        یک استریم send تولید می‌کند (برای ریپلیکیشن/بکاپ). پیشنهاد می‌شود برای استریم‌های بزرگ به فایل نوشته شود.
        ورودی‌ها:
          - snapshot (str): نام اسنپ‌شات مبدأ (مانند "tank/data@A").
          - incremental_from (str|None): اگر داده شود، استریم افزایشی از این اسنپ‌شات مبنا تولید می‌شود (-I).
        ...
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» یا تایم‌اوت.
    ...
    def receive(self, target: str, input_file: Optional[str] = None, stdin_bytes: Optional[bytes] = None,
        ...
        """
        استریم send را در مقصد دریافت می‌کند. منبع می‌تواند فایل یا دادهٔ باینری در حافظه باشد.
        ورودی‌ها:
        ...
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» یا ورودی نامعتبر (هر دو منبع داده داده شده باشد).
    ...
    def diff(self, older: str, newer: str) -> Dict[str, Any]:  # مقایسهٔ تغییرات.
        """
        خروجی `zfs diff` را بین دو نقطه (اسنپ‌شات/دیتاست) بازمی‌گرداند.
        ورودی‌ها:
    ...
    def history(self, dataset_or_pool: Optional[str] = None) -> Dict[str, Any]:  # تاریخچهٔ عملیات.
        """
        تاریخچهٔ عملیات «زِد اِف اِس» را بازمی‌گرداند (در سطح گلوبال یا محدود به یک پول/دیتاست).
        ورودی:
    ...
    def export_full_state(self) -> Dict[str, Any]:  # خروجی جامع وضعیت «زِد اِف اِس».
        """
        نمایی کامل و عمیق از وضعیت «زِد اِف اِس» بازمی‌گرداند تا در داشبورد و نظارت استفاده شود. شامل:
          - اطلاعات هر pool: نام، guid، state/health، پراپرتی‌های کلیدی، featureها، خروجی خام status -v، و iostat.
          - دیتاست‌های زیر هر پول: نوع، تمام پراپرتی‌ها، لیست اسنپ‌شاتها و بوک‌مارکها.
          - خلاصه‌ای از تمام اسنپ‌شاتهای موجود در کل سیستم.
        ورودی:
          - ندارد.
        خروجی:
