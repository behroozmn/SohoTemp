...
و در جاهایی که API کامل در دسترس نیست با fallback ایمن به «کامندلاین»های رسمی (zfs/zpool) عملیات روی «زِد اِف اِس» را انجام می‌دهد.
...
import subprocess  # اجرای امن فرمان‌های سیستمی («زِد اِف اِس»/«زِد پول») وقتی API کامل نیست یا برای گزارش خام.
...
class ZFSManager:  # کلاس مدیریت سطح‌بالای «زِد اِف اِس» با خروجی JSON-Ready و fallback به «کامندلاین».
    """
    هدف: ارائهٔ API سطح‌بالا برای مدیریت «زِد اِف اِس» که
      1) تا جای ممکن از libzfs برای کارهای داخل حافظه (introspection و set/get properties) استفاده کند.
      2) در جاهایی که API پوشش ندارد (مثل send/receive یا create pool)، با اجرای امن «کامندلاین» (zfs/zpool) کار را انجام دهد.
    ...
    def _run(self, args: List[str], stdin: Optional[bytes] = None) -> Tuple[str, str]:  # اجرای امن فرمان «کامندلاین» با آرگومان‌های لیستی.
        """
        این متد داخلی فرمان‌های «کامندلاین» مثل zfs/zpool را به‌صورت ایمن اجرا می‌کند.
