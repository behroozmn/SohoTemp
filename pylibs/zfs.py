...
def ok(data: Any, meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # تابع کمکی برای ساخت پاسخ موفق استاندارد.
    """
    این تابع یک پوشش (envelope) استاندارد برای پاسخ موفق تولید می‌کند تا به‌صورت مستقیم در DRF استفاده شود.
    ورودی‌ها:
...
class ZFSError(Exception):  # استثنای اختصاصی دامنهٔ «زِد اِف اِس» برای تمایز خطاها در لاجیک برنامه.
    ...
    چرا؟ برای اینکه بتوانیم خطاهای مربوط به «زِد اِف اِس» را از سایر خطاها جدا کنیم و در handlerهای DRF واکنش مناسب نشان دهیم.
    ورودی‌ها/خروجی:
