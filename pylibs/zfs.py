...
class ZFSManager:  # کلاس مدیریت سطح‌بالای «زِد اِف اِس» با خروجی JSON-Ready و fallback به «کامندلاین».
    ...
    def list_datasets(self, pool: Optional[str] = None,
        ...
        """
        فهرست دیتاست‌ها (filesystem, volume(zvol), snapshot) را به همراه نوعشان بازمی‌گرداند.
        ورودی‌ها:
    ...
    def create_dataset(self, name: str, properties: Optional[Dict[str, str]] = None,
        ...
        """
        یک دیتاست جدید می‌سازد؛ اگر نوع "volume" انتخاب شود، باید حتماً پراپرتی "volsize" تعیین شود.
        ورودی‌ها:
        ...
        خطاها:
          - ZFSError: خطا در اجرای «کامندلاین» (مانند نبودن volsize برای volume).
    ...
    def mount(self, dataset: str) -> Dict[str, Any]:  # مونت دیتاست.
        """
        دیتاست نوع filesystem را طبق mountpoint خودش سوار می‌کند.
        ورودی:
          - dataset (str): نام دیتاست.
        خروجی:
    ...
    def set_record_or_volblock(self, dataset: str, size: str = "128K") -> Dict[str, Any]:  # تعیین recordsize/volblocksize.
        """
        اگر دیتاست از نوع zvol باشد volblocksize تنظیم می‌شود و اگر filesystem باشد recordsize.
        ورودی‌ها:
          - dataset (str): نام دیتاست.
    ...
    def history(self, dataset_or_pool: Optional[str] = None) -> Dict[str, Any]:  # تاریخچهٔ عملیات.
        """
        تاریخچهٔ عملیات «زِد اِف اِس» را بازمی‌گرداند (در سطح global یا محدود به یک pool/dataset).
        ورودی:
