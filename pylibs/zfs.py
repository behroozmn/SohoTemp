...
و در جاهایی که API کامل در دسترس نیست با fallback ایمن به CLIهای رسمی (zfs/zpool) عملیات روی «زِد اِف اِس» را انجام می‌دهد.
...
class ZFSManager:  # کلاس مدیریت سطح‌بالای «زِد اِف اِس» با خروجی JSON-Ready و fallback به CLI.
    """
    ...
    def _run(self, args: List[str], stdin: Optional[bytes] = None) -> Tuple[str, str]:  # اجرای امن فرمان CLI با آرگومان‌های لیستی.
        """
        این متد داخلی فرمان‌های CLI مثل zfs/zpool را به‌صورت ایمن اجرا می‌کند.
    ...
    def _get_pool(self, name: str) -> libzfs.ZFSPool:  # دریافت شیء zpool بر اساس نام.
        """
        در بین zpoolها پیمایش می‌کند و پول با نام داده‌شده را برمی‌گرداند.
        ورودی:
    ...
    def list_pools(self) -> Dict[str, Any]:  # لیست نام تمام zpoolها.
        """
        نام تمام zpoolهای موجود در سیستم را از libzfs می‌خواند.
        ورودی:
          - ندارد.
        خروجی:
    ...
    def pool_status(self, pool: str) -> Dict[str, Any]:  # وضعیت سادهٔ یک zpool.
        """
        وضعیت پایهٔ یک zpool شامل نام، state، health، guid و تعدادی پراپرتی رایج را برمی‌گرداند.
        ورودی:
    ...
    def get_props(self, target: str) -> Dict[str, Any]:  # گرفتن همهٔ پراپرتی‌ها برای یک دیتاست.
        """
        تمام پراپرتی‌های قابل مشاهدهٔ یک دیتاست را (ترجیحاً با libzfs و در غیر اینصورت با CLI) بازمی‌گرداند.
        ورودی:
    ...
    def create_pool(self, name: str, vdevs: List[List[str]],
        ...
        """
        یک zpool جدید می‌سازد. برای vdevها باید آرایه‌ای از گروه‌ها بدهید (مثلاً [["mirror","/dev/sdb","/dev/sdc"], ["raidz1",...]]).
        ورودی‌ها:
    ...
    def destroy_pool(self, name: str, force: bool = False) -> Dict[str, Any]:  # حذف zpool (خطرناک).
        """
        یک zpool را به‌طور کامل حذف می‌کند. بسیار خطرناک است و همهٔ داده‌ها از دست می‌رود.
        ورودی‌ها:
    ...
    def import_pool(self, name: Optional[str] = None,
        ...
        """
        ایمپورت یک zpool از دیسک‌ها. می‌توانید مسیر جستجو بدهید یا حالت فقط‌خواندنی تنظیم کنید.
        ورودی‌ها:
    ...
    def export_pool(self, name: str, force: bool = False) -> Dict[str, Any]:  # اکسپورت پول.
        """
        zpool را از سیستم جاری خارج (export) می‌کند تا در سیستم دیگری import شود.
        ورودی‌ها:
          - name (str): نام پول.
    ...
    def set_props(self, target: str, properties: Dict[str, str]) -> Dict[str, Any]:  # ست‌کردن پراپرتی‌ها.
        """
        پراپرتی‌های یک دیتاست را تنظیم می‌کند؛ ابتدا سعی با libzfs و در صورت نیاز با CLI.
        ورودی‌ها:
